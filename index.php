      ...
      <div class="row justify-content-center">
        <div class="col-xl-10">
          ...
          <div class="app-hero">
            <div class="app-hero__content">
              <div class="app-badge">
                ...
                LGU Appointment System
              </div>
              <h1 class="app-title">Book your appointment faster.</h1>
              <p class="app-subtitle">
                Please provide your client information to continue. Your details help us validate your request and process it efficiently.
              </p>
            </div>
              ...
              <form method="post" action="select.php" novalidate class="app-form">
                ...
                <div class="row g-3">
                  <div class="col-md-4">
                    ...
                    <input name="first_name" class="form-control app-input" required value="<?= h($old['first_name']) ?>" autocomplete="given-name">
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">Middle Name</label>
                    <input name="middle_name" class="form-control app-input" value="<?= h($old['middle_name']) ?>" autocomplete="additional-name">
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">Last Name <span class="req">*</span></label>
                    <input name="last_name" class="form-control app-input" required value="<?= h($old['last_name']) ?>" autocomplete="family-name">
                  </div>
                  ...
                  <div class="col-12">
                    ...
                    <input type="hidden" name="address" id="address_full" value="<?= h($old['address']) ?>">
                  </div>

                  <div class="col-md-6">
                    <label class="form-label">CP Number <span class="req">*</span></label>
                    <input name="cp_no" class="form-control app-input" required value="<?= h($old['cp_no']) ?>" placeholder="e.g. 09xxxxxxxxx" autocomplete="tel">
                    <div class="app-help mt-1">Use an active number for updates and confirmations.</div>
                  </div>
                </div>
          ...
          <footer class="app-footer">
            <div class="small">
              © <?= date('Y') ?> Local Government Unit • Appointment Services
            </div>
          </footer>
...
<script src="assets/js/bootstrap.bundle.min.js"></script>
  <!-- KEEP YOUR PSGC JS BELOW (unchanged) -->
  <script>
  /* ===============================
    PSGC Loader (via api/psgc.php proxy)
  ================================== */
  ...
  function normalizeItems(items) {
    ...
    if (items && Array.isArray(items.data)) return items.data; // supports {data:[...]}
    return [];
  }
  ...
  function rebuildFullAddress() {
    const brgy = selectedText(elBrgy);
    const cm   = selectedText(elCityMun);
    const prov = selectedText(elProv);
    const reg  = selectedText(elRegion);
